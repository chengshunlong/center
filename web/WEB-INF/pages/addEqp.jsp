...
<div class="container">
    ...
    <div class="row">
        <form action="${pageContext.request.contextPath}/eqp/addEqp" method="post">
            <div class="form-group">
                <label for="eqpName">器材名称</label>
                <input type="text" class="form-control" value="${errorEqp.eqpName}" name="eqpName" id="eqpName" required onblur="updateEqpNameVerify()">
                <span id="eqpNameReturnInfo" class="help-block">2-16位非特殊字符</span>
            </div>
            <div class="form-group">
                <label for="eqpPrice">器材价格</label>
                <input type="text" class="form-control" value="${errorEqp.eqpPrice}" name="eqpPrice" id="eqpPrice" maxlength="8" required onblur="updateEqpPriceVerify()">
                <span id="eqpPriceReturnInfo" class="help-block">0-9999以内的整数</span>
            </div>
            <div class="form-group">
                <label for="eqpAmount">器材数量</label>
                <input type="text" class="form-control" value="${errorEqp.eqpAmount}" name="eqpAmount" id="eqpAmount" maxlength="8" required onblur="updateEqpAmountVerify()">
                <span id="eqpAmountReturnInfo" class="help-block">0-9999以内的整数</span>
            </div>
<%--            <div class="form-group">--%>
<%--                <label for="eqpPicture">器材图片</label>--%>
<%--                <input type="text" class="form-control" name="eqpPicture" id="eqpPicture" required>--%>
<%--            </div>--%>
            <button type="submit" class="btn btn-default">确定</button>
        </form>
    </div>
    <div class="row">
        <c:if test="${requestScope.errorMsg == '新增失败！请检查输入的数据是否合法'}">
